...
const { agent_assessments, agencies } = require('../model/texPayersmodels');
...
module.exports.adminDashboard = async (req, res) => {
    try {
        ...
        let consultants = await agencies.count()
        let offices = await tax_offices.count()

        let officSucc = await assessments.count({ where: { settlement_status: 1 } })
        let revenueOficePaid = await assessments.sum('assessment_amount_paid', { where: { settlement_status: 1 } })
        let revenueOficeUnPaid = await assessments.sum('assessment_amount', { where: { settlement_status: 0 } })


        let consultPaid = await agent_assessments.sum('amount',  { where: { settlement_status: 1 }})
        let consultUpiad = await agent_assessments.sum('amount',  { where: { settlement_status: 0 }})
        let paidCount = await agent_assessments.count({ where: { settlement_status: 1 }})
        res.render('./CBS/dashboard', {
            individualCount,
            companyCount, revenueOficePaid, revenueOficeUnPaid, officSucc, consultPaid,
            consultUpiad,
            paidCount, consultants,
            offices
        })
    } catch (err) {
        console.log(err)
