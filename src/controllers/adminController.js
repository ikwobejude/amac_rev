...
module.exports = {
    ...
    createOffices: async (req, res) => {
        try {
            ...
            if (error) {
                ...
                try {
                    let offices = await tax_offices.findOne({where: {tax_office:value.tax_office }});
                    if(offices){
                        res.status(201).json({ err: "Tax office already exist" });
                    } else {
    ...
    addUserTofficeAsStaff: async (req, res) => {
        try {
            let value = req.body
    ...
    updateAgencyInfo1: async (req, res) => {
        try {
